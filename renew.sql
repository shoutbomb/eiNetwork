SELECT 'p' || rmp.record_num || 'a' AS patron_no, 
...
RIGHT JOIN sierra_view.patron_view AS p 
...
WHERE
(c.due_gmt::date-current_date) in (0) AND c.loanrule_code_num IN (%CourtesyNotice1DayBefore%)
...
ON ( rmb.id = b.id AND rmb.record_type_code = 'b') 
WHERE
(c.due_gmt::date-current_date) in (0,1,2) AND c.loanrule_code_num NOT IN (%CourtesyNotice1DayBefore%)
GROUP BY 1,2,3,4,5,6,7,10,11,12,13
ORDER BY patron_no ;